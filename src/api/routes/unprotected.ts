...
const UnprotectedRoutes = new Hono()
	.post("/login", validateJson(loginSchema), async (c) => {
		...
		try {
			...
			const [users] = await conn.execute<(DB.AimeUser & RowDataPacket)[]>("SELECT * FROM aime_user WHERE username = ?", [
				username,
			]);
	...
	.post("/signup", validateJson(signupSchema), async (c) => {
		...
		try {
			...
			const [updatedUsers] = await conn.execute<(DB.AimeUser & RowDataPacket)[]>("SELECT * FROM aime_user WHERE id = ?", [
				userId,
			]);
