...
export function useUpdateArcadeName() {
	const queryClient = useQueryClient()

	return useMutation({
		mutationFn: async ({ arcade, name, nickname }: { arcade: number; name?: string; nickname?: string }) => {
			const response = await api.arcades.update.name.$post({
				json: { arcade, name, nickname }
			})

			if (!response.ok) {
				throw new Error("Failed to update arcade name")
			}
			return await response.json()
		},
		onSuccess: () => {
