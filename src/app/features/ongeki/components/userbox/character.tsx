...
import {
	...
	useSearchCharacters,
	useUnlockCharacter
} from "@/app/features/ongeki/hooks/userbox/character"
		...
		<>
				...
				<div className="flex flex-1 flex-col p-4 text-center">
					...
					<div className="mb-auto flex flex-1 items-center justify-center">
						{currentCharacter?.imagePath ? (
							<img
								src={`${CDN}/ongeki/chara/${currentCharacter.imagePath}`}
								alt="Character"
								className="h-20 w-20 rounded-sm object-cover"
							/>
						) : (
							<div className="bg-muted flex h-20 w-20 items-center justify-center rounded-sm">
			...
			<ItemSelectionDialog
				...
				items={items.map(item => ({
					id: item.characterId,
					name: item.label,
					imageUrl: `${CDN}/ongeki/chara/${item.imagePath}`,
					locked: item.locked
				}))}
