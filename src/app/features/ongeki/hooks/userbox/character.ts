...
export function useCurrentCharacter() {
	return useQuery({
		queryKey: ["ongeki", "userbox", "character", "current"],
		queryFn: async () => {
			const response = await api.ongeki.userbox.character.$get()
			if (!response.ok) {
...
export function useSearchCharacters(filters: { locked: boolean | null }) {
	return useQuery({
		queryKey: ["ongeki", "userbox", "character", "search", filters],
		queryFn: async () => {
			const response = await api.ongeki.userbox.character.search.$post({
				json: { filter: filters }
			})
...
export function useEquipCharacter() {
	...
	return useMutation({
		mutationFn: async (id: number) => {
			...
			const searchQueries = queryClient.getQueriesData({ queryKey: ["ongeki", "userbox", "character", "search"] })
			...
			const response = await api.ongeki.userbox.character.$post({
		...
		onSuccess: (data, id) => {
			...
			queryClient.invalidateQueries({ queryKey: ["ongeki", "userbox", "character", "current"] })

			// Update current character in cache with server response
			queryClient.setQueryData(["ongeki", "userbox", "character", "current"], data)

			// Update search results to reflect new equipped status
			queryClient.setQueriesData({ queryKey: ["ongeki", "userbox", "character", "search"] }, (old: any) => {
			...
			queryClient.invalidateQueries({ queryKey: ["ongeki", "userbox", "character", "search"] })
		}
	})
}
...
export function useUnlockCharacter() {
	...
	return useMutation({
		mutationFn: async (id: number) => {
			...
			const response = await api.ongeki.userbox.character.unlock[":characterId"].$patch({
